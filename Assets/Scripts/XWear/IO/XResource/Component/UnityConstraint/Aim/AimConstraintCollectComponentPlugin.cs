...
{
    public class AimConstraintCollectComponentPlugin
        : DefaultCollectComponentPluginBase<AimConstraint>
    {
        public override int Order => 10;

        protected override void CopyComponent(
            UnityEngine.Transform attachTarget,
            AimConstraint sourceComponent
        )
        {
            var result = attachTarget.gameObject.AddComponent<AimConstraint>();
            result.constraintActive = sourceComponent.constraintActive;
            result.weight = sourceComponent.weight;
            result.aimVector = sourceComponent.aimVector;
            result.upVector = sourceComponent.upVector;
            result.worldUpType = sourceComponent.worldUpType;
            result.worldUpVector = sourceComponent.worldUpVector;
        ...
        )
        {
            ...
            var result = new XResourceAimConstraint(constraint, gameObjectCollector);
            return result;
        }
    }
}
