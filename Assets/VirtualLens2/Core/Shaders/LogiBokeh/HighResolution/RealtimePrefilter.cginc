#ifndef VIRTUALLENS2_LOGIBOKEH_HIGH_RESOLUTION_REALTIME_PREFILTER_CGINC
#define VIRTUALLENS2_LOGIBOKEH_HIGH_RESOLUTION_REALTIME_PREFILTER_CGINC
...
float4 fragment(v2f i) : SV_Target {
	if(i.uv.x < 0.5){
		return downsampleBackground(i.uv * float2(2.0, 2.0) - float2(0.0, 0.0));
	}else{
		return downsampleForeground(i.uv * float2(2.0, 2.0) - float2(1.0, 0.0));
	}
}
