#ifndef VIRTUALLENS2_LOGIBOKEH_HIGH_RESOLUTION_REALTIME_TILE_CONFIG_CGINC
#define VIRTUALLENS2_LOGIBOKEH_HIGH_RESOLUTION_REALTIME_TILE_CONFIG_CGINC
...
float4 fetchTile(float2 uv){
	const float2 minval = float2(0.0, 4.0 / 5.0);
...
float4 fetchDilatedTile(float2 uv){
	const float2 minval = float2(0.5, 4.0 / 5.0);
