#ifndef VIRTUALLENS2_LOGIBOKEH_HIGH_RESOLUTION_REALTIME_POSTFILTER_CGINC
#define VIRTUALLENS2_LOGIBOKEH_HIGH_RESOLUTION_REALTIME_POSTFILTER_CGINC
...
float4 fragment(v2f i) : SV_Target {
	if(i.uv.x < 0.5){
		...
		return backgroundPostfilter(uv, i.uv);
	}else{
		const float2 uv = i.uv * float2(2.0, 2.0) - float2(1.0, 0.0);
		if(!inRequiredRange(uv)){ discard; }
		return foregroundPostfilter(uv, i.uv);
	}
}
