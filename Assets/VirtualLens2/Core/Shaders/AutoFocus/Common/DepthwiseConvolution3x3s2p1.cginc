//---------------------------------------------------------------------------
// DepthwiseConvolution3x3 (stride=2, pad=1)
//
// Parameters:
// - BUFFER_WIDTH
// - BUFFER_HEIGHT
...
static const uint OUTPUT_WIDTH    = ceil_div(INPUT_WIDTH,  2u);
static const uint OUTPUT_HEIGHT   = ceil_div(INPUT_HEIGHT, 2u);
...
float4 fragment(v2f i) : SV_TARGET {
	...
	const bool x_hi_edge = (out_x0 * 4u + 2u == INPUT_WIDTH  - 1u);
	const bool y_hi_edge = (out_y0 * 4u + 2u == INPUT_HEIGHT - 1u);
...
#define ACCUMULATE(dst, s, w) \
	...
	if(!y_lo_edge){
		...
		}
		{
			...
			ACCUMULATE(acc01, input.w, w00);
		}
		if(!x_hi_edge){
			const float4 input = load_input(index0 - y_step + x_step);
			ACCUMULATE(acc01, input.z, w01);
			ACCUMULATE(acc01, input.w, w02);
		}
	}
	{
		if(!x_lo_edge){
		...
		}
		{
			...
			ACCUMULATE(acc01, input.y, w10);
			ACCUMULATE(acc01, input.w, w20);
			ACCUMULATE(acc10, input.z, w01);
			ACCUMULATE(acc10, input.w, w02);
			ACCUMULATE(acc11, input.w, w00);
		}
		if(!x_hi_edge){
			const float4 input = load_input(index0 + x_step);
			ACCUMULATE(acc01, input.x, w11);
			ACCUMULATE(acc01, input.y, w12);
			ACCUMULATE(acc01, input.z, w21);
			ACCUMULATE(acc01, input.w, w22);
			ACCUMULATE(acc11, input.z, w01);
			ACCUMULATE(acc11, input.w, w02);
		}
	}
	if(!y_hi_edge){
		if(!x_lo_edge){
			const float4 input = load_input(index0 + y_step - x_step);
			ACCUMULATE(acc10, input.y, w10);
			ACCUMULATE(acc10, input.w, w20);
		}
		{
			const float4 input = load_input(index0 + y_step);
			ACCUMULATE(acc10, input.x, w11);
			ACCUMULATE(acc10, input.y, w12);
			ACCUMULATE(acc10, input.z, w21);
			ACCUMULATE(acc10, input.w, w22);
			ACCUMULATE(acc11, input.y, w10);
			ACCUMULATE(acc11, input.w, w20);
		}
		if(!x_hi_edge){
			const float4 input = load_input(index0 + y_step + x_step);
			ACCUMULATE(acc11, input.x, w11);
			ACCUMULATE(acc11, input.y, w12);
			ACCUMULATE(acc11, input.z, w21);
			ACCUMULATE(acc11, input.w, w22);
		}
	}
